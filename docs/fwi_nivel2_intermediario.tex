...
\ponte{N\'ivel 3, §6.5}{a mesma dedu\c{c}\~ao em espa\c{c}os de
...
\times \underbrace{2}_{\text{direto + adjunto}}
\times \underbrace{(1 + n_{\text{busca}})}_{\text{busca linear}}
\times \text{custo}_{\text{modelagem}} .
\label{eq:custo}
\end{equation}

O fator 2 \'e o pre\c{c}o do gradiente adjunto (Cap.~\ref{cap:adjunto}); o
fator da busca linear costuma valer entre 1 e 3 (Cap.~\ref{cap:otimizacao}).
Com 30 itera\c{c}\~oes, 3 bandas, 200 tiros e busca linear de 2
avalia\c{c}\~oes, isso d\'a cerca de $30 \times 3 \times 200 \times 2 \times 3
= 108\,000$ modelagens completas. Se cada uma custa 10 segundos, s\~ao
12{,}5 dias de m\'aquina --- em um n\'ucleo.
...
\textbf{Profundidade de investiga\c{c}\~ao.} Governada pela abertura:
\begin{equation}
z_{\max} \approx \frac{x_{\text{off}}^{\max}}{3}\ \text{a}\
\frac{x_{\text{off}}^{\max}}{2}.
\end{equation}
...
avalia\c{c}\~oes?
\end{enumerate}
\textbf{Respostas.} (1) Entre 2 e 3\,km. (2) A banda \'util vai at\'e
$\approx 1{,}6 \times 8 = 12{,}8$\,Hz; $\delta_{\min} = 2500/(2\cdot 12{,}8)
\approx 98$\,m. (3) De \eqref{eq:custo}: $50\,000 = n_{\text{iter}} \times 3
\times 100 \times 2 \times 3 \Rightarrow n_{\text{iter}} \approx 28$ por
banda.
\end{exercicio}
...
Praticamente todo c\'odigo de produ\c{c}\~ao (incluindo o PyFWI, usado no
curso que acompanha esta cole\c{c}\~ao) resolve exatamente
\eqref{eq:momento}--\eqref{eq:hooke} na forma \emph{velocidade--tens\~ao} de
primeira ordem, em malha intercalada. O regime ac\'ustico \'e obtido
...
\[
\eta = 0{,}5\ (-6\,\text{dB}):\ f \approx 1{,}63 f_0,
\qquad
...
Em $\mathcal{O}(8)$ o efeito \'e dram\'atico: com $G=5$ e $C=0{,}5$ a curva
espacial promete $0{,}07\%$ e o esquema completo entrega $1{,}6\%$ --- porque
...
A FWI trabalha \emph{sempre} em \emph{shot gathers}, e a raz\~ao \'e
conceitual: $F$ simula um tiro por vez, ent\~ao o res\'iduo
$F(\mvec) - \dobs$ tem de ser calculado nessa organiza\c{c}\~ao. \'E
tamb\'em por isso que a FWI n\~ao usa dado empilhado --- o empilhamento
descarta a depend\^encia com o afastamento, que \'e justamente a
...
\item \textbf{Reflex\~oes}: $t(x)^{2} = t_0^{2} + x^{2}/v_{\text{rms}}^{2}$
--- hip\'erboles cuja curvatura revela a velocidade m\'edia.
...
\[
z_{\max} \;\approx\; \frac{x_{\text{off}}^{\max}}{3}
\ \text{a}\ \frac{x_{\text{off}}^{\max}}{2}
\]
...
\textbf{uma fun\c{c}\~ao convexa tem um \'unico m\'inimo}, e qualquer
m\'etodo que desce pelo gradiente chega nele, venha de onde vier.
...
comparou com \eqref{eq:tolerancia}:
...
$L_2$ cl\'assico & $\tfrac12\int (d_{\text{cal}}-d_{\text{obs}})^{2}$
  & \emph{cycle skipping} \\
Envolt\'oria & compara $\abs{\mathcal{H}\{d\}}$ (envolt\'oria de Hilbert),
  que \'e insens\'ivel \`a fase fina & perde resolu\c{c}\~ao \\
Correla\c{c}\~ao cruzada & mede o \emph{deslocamento} $\tau^{*}$ que maximiza
  a correla\c{c}\~ao e penaliza $\tau^{*2}$ & precisa janelar \\
...
dominado pela curvatura. Use uma dire\c{c}\~ao \textbf{suave} --- o pr\'oprio
...
pontos percentuais. E isso \'e esperado: o modelo inicial tem apenas 8\% de
erro e correla\c{c}\~ao $0{,}93$ com o verdadeiro, ou seja, j\'a est\'a
\emph{dentro} do vale de atra\c{c}\~ao de 25\,Hz. Quando o monoescala n\~ao
sofre \emph{cycle skipping}, n\~ao h\'a muito para o multiescala resgatar.
...
Regulariza\c{c}\~ao \textbf{n\~ao} melhora o ajuste dos dados: no \'otimo, por
constru\c{c}\~ao, $J_{\text{dados}}$ s\'o pode ficar igual ou \emph{pior} do
que sem ela. O que ela faz \'e escolher, entre os muitos modelos que ajustam
os dados igualmente bem, aquele mais plaus\'ivel segundo um crit\'erio que
\emph{voc\^e} definiu.
...
\caption{Curva em L medida pelo c\'odigo do curso. O cotovelo \'e o
compromisso: \`a esquerda, sobre-regulariza\c{c}\~ao (ajuste sacrificado);
\`a direita, ru\'ido entrando no modelo.}
\end{figure}
...
homog\^enea; com $\varepsilon$ grande, $\sqrt{\abs{\nabla m}^2+\varepsilon^2}
\approx \varepsilon$ e a TV degenera em Tikhonov. (3) Qualquer um dos dois
...
17\% menor d\'a reflex\~ao praticamente \textbf{nula} --- uma FWI ac\'ustica
...
\ponte{N\'ivel 3, Cap.\ 11}{o tratamento quantitativo disso: padr\~oes de
...
causal, a sua transformada \'e anal\'itica no semiplano superior, e as partes
real e imagin\'aria deixam de ser independentes --- ficam ligadas pelas
...
Penetra\c{c}\~ao & $z_{\max} \approx x_{\text{off}}^{\max}/3$ a
  $x_{\text{off}}^{\max}/2$ \\
...
Custo total & $n_{\text{iter}} n_{\text{bandas}} n_{\text{tiros}}
  \cdot 2 \cdot (1+n_{\text{busca}})$ modelagens \\
