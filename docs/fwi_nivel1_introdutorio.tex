...
tr\'as --- e o \textbf{n\'umero de simula\c{c}\~oes n\~ao depende do n\'umero
de c\'elulas}. (Cada simula\c{c}\~ao, claro, fica mais cara numa malha maior;
o que some \'e o fator ``um milh\~ao''.) \'E a diferen\c{c}a entre imposs\'ivel e
rotineiro.
\end{teoria}
...
sexto e um ter\c{c}o do maior afastamento fonte--receptor (Morgan \emph{et
al.}, 2013);
...
um s\'o. De tr\^es par\^ametros f\'isicos ($v_p$, $v_s$, $\rho$) para dois
($v_p$, $\rho$) --- e, adotando tamb\'em densidade constante, como quase sempre
se faz, para um s\'o ($v_p$). O custo computacional cai por um fator de
...
A hip\'otese \eqref{eq:mu0} leva a uma \'unica equa\c{c}\~ao escalar para a
press\~ao. Somando a ela uma simplifica\c{c}\~ao de uso quase universal ---
\textbf{densidade constante}, que o Cap\'itulo~\ref{cap:limites} volta a
discutir ---, chega-se \`a equa\c{c}\~ao que governa praticamente tudo nesta
cole\c{c}\~ao:
\begin{equation}
...
Parece detalhe est\'etico. N\~ao \'e: \'e a raz\~ao pela qual muitos
c\'odigos de FWI calculam o gradiente em $m = 1/c^{2}$ e s\'o depois o convertem
para velocidade. O Cap\'itulo~\ref{cap:adjunto}
...
tamanho determina a \textbf{ordem} do esquema. Um est\^encil de 3 pontos por
...
f\'isica viaja a dist\^ancia $c\,\Delta t$. Num esquema que s\'o consulta os
vizinhos imediatos, se essa dist\^ancia passar de uma c\'elula, a onda estaria
``pulando por cima'' de pontos que o esquema nunca consultou --- e o resultado
n\~ao tem como estar certo. A conta vira um n\'umero adimensional, o
\textbf{n\'umero de Courant}:
\begin{equation}
...
O argumento geom\'etrico d\'a apenas uma condi\c{c}\~ao \textbf{necess\'aria},
e n\~ao conta a hist\'oria toda. Para ordem 2 em 2D ele acerta em cheio
($0{,}71 = 1/\sqrt{2}$, a meia diagonal da c\'elula); mas um est\^encil de
ordem 8 alcan\c{c}a quatro c\'elulas, e mesmo assim o limite \emph{cai}. O
limite verdadeiro vem de uma an\'alise de estabilidade, que o Volume 2 faz.
...
\item \textbf{reflex\~oes} --- hip\'erboles: a curvatura revela uma velocidade
m\'edia at\'e o refletor (a m\'edia RMS, que pesa mais as camadas r\'apidas);
...
\frac{\text{afastamento m\'aximo}}{6}\ \text{a}\
\frac{\text{afastamento m\'aximo}}{3}.
\]
(Morgan \emph{et al.}, 2013. O valor exato dentro da faixa depende de quanto a
velocidade cresce com a profundidade.)

A raz\~ao \'e geom\'etrica. A velocidade de grande escala vem sobretudo das
ondas que \emph{mergulham} e voltam \`a superf\'icie, e uma onda que volta a
uma dist\^ancia $x$ da fonte n\~ao desceu mais que uma fra\c{c}\~ao de $x$. Al\'em
disso, para separar ``mais lento e mais raso'' de ``mais r\'apido e mais
fundo'' \'e preciso iluminar cada ponto com \^angulos \emph{variados}, e
\^angulos variados exigem raios vindo de longe. Afastamento curto s\'o produz
incid\^encia quase normal, que \'e amb\'igua.
...
\begin{atencao}[Por que a FWI n\~ao usa dado empilhado]
A FWI compara registros de \emph{disparo}, e a raz\~ao \'e conceitual: o
simulador $F$ modela disparos, ent\~ao o res\'iduo tem de ser calculado nessa
mesma organiza\c{c}\~ao --- um disparo por vez ou, em t\'ecnicas de
acelera\c{c}\~ao, somas de disparos que o simulador reproduz \emph{exatamente}
(a codifica\c{c}\~ao de fontes do Volume 3).

O que ela n\~ao usa \'e o dado empilhado do processamento convencional: o
empilhamento descarta justamente a depend\^encia com o afastamento --- que \'e
a informa\c{c}\~ao que separa velocidade de profundidade.
\end{atencao}
...
precisam concordar.
\item A FWI compara registros de disparo, nunca dado empilhado.
\item Profundidade de investiga\c{c}\~ao $\approx$ um sexto a um ter\c{c}o do
...
Transporte \'otimo & dist\^ancia entre ``massas'' de sinal & sinal precisa virar massa positiva \\
...
\item \textbf{O n\'umero de simula\c{c}\~oes n\~ao depende do n\'umero de
c\'elulas.} Duas simula\c{c}\~oes por disparo, e o gradiente inteiro sai. Foi essa
...
\ponte{N\'ivel 3, §3.4}{o adjunto discreto \emph{versus} o cont\'inuo, e por
...
Newton & a curvatura exata (Hessiana) & montar a matriz \'e imposs\'ivel
($10^{12}$ entradas); a vers\~ao \emph{truncada} s\'o usa produtos dela por
vetores (Volume 3) \\
\bottomrule
\end{tabular}
\end{center}
...
esperado: o pr\'oprio monoescala \emph{melhorou} o modelo, sinal de que o
inicial j\'a estava dentro do vale de atra\c{c}\~ao de 25\,Hz. Quando o
monoescala n\~ao sofre \emph{cycle skipping}, n\~ao h\'a muito para a
multiescala resgatar.

(Isso n\~ao se deduz do n\'umero 8\% sozinho. Erro relativo de modelo \'e uma
medida global, que mistura todas as escalas; o crit\'erio do
Cap\'itulo~\ref{cap:misfit} fala do erro de \emph{tempo} acumulado no percurso.
Um inicial suave pode ter 8\% de erro relativo e cinem\'atica quase certa.)
...
Comparando os \emph{melhores} modelos poss\'iveis com e sem ela, por
constru\c{c}\~ao $J_{\text{dados}}$ s\'o pode ficar igual ou \emph{pior}.
...
(Uma ressalva, porque a FWI tem m\'inimos locais: partindo do mesmo modelo
inicial, uma regulariza\c{c}\~ao pode desviar a invers\~ao de um vale errado e
terminar com ajuste \emph{melhor} do que a vers\~ao sem ela. Isso \'e efeito do
caminho percorrido, n\~ao uma contradi\c{c}\~ao da frase acima.)
\end{atencao}
...
\item Regularizar n\~ao serve para melhorar o ajuste dos dados --- escolhe entre modelos
que ajustam igual.
...
O que controla a intensidade de uma reflex\~ao em incid\^encia normal n\~ao
\'e a velocidade sozinha: \'e a \textbf{imped\^ancia}, o produto densidade
$\times$ velocidade. A consequ\^encia \'e imediata e inc\^omoda:

\begin{center}
\begin{tabular}{@{}llr@{}}
\toprule
\textbf{Meio 1} & \textbf{Meio 2} & \textbf{Reflex\~ao (incid\^encia normal)} \\
\midrule
...
17\% menor produz reflex\~ao praticamente \textbf{nula} --- nas reflex\~oes de
afastamento curto, essa interface simplesmente n\~ao aparece. S\'o se chega a
ela por outros caminhos: o atraso das ondas que a atravessam, ou a
varia\c{c}\~ao da reflex\~ao com o \^angulo, que exige afastamento.
...
\begin{atencao}[O custo da tabela \'e na mesma malha]
Os fatores acima comparam uma simula\c{c}\~ao com a outra \emph{na mesma
grade}. Mas a grade \'e ditada pela menor velocidade
(Cap.~\ref{cap:numerico}), e na el\'astica ela \'e a da onda S. Com
$v_p/v_s = 2$, o espa\c{c}amento cai pela metade: mais $8\times$ em 2D e
$16\times$ em 3D, \emph{em cima} do fator da tabela. Em sedimento raso e mole,
com $v_s$ muito baixo, a diferen\c{c}a chega a ordens de grandeza.
\end{atencao}
...
invis\'ivel nas reflex\~oes de incid\^encia normal.
...
receptores. \'E a organiza\c{c}\~ao natural do dado em FWI.
...
\item LAILLY, P. \emph{The seismic inverse problem as a sequence of before
stack migrations.} In: BEDNAR, J.~B. \emph{et al.} (ed.). \emph{Conference on
Inverse Scattering: Theory and Application}. Philadelphia: SIAM, 1983.
p.~206--220.

\item TARANTOLA, A. \emph{Inversion of seismic reflection data in the acoustic
approximation.} Geophysics, v.~49, n.~8, p.~1259--1266, 1984. --- com
Lailly (1983), o ponto de partida da FWI moderna.
...
Geophysical Journal International, v.~195, n.~3, p.~1657--1678, 2013. --- a
regra de profundidade de penetra\c{c}\~ao.
...
--- a refer\^encia cl\'assica da estrat\'egia multiescala.
