...
Cap\'itulo~\ref{cap:verificacao} mede exatamente \eqref{eq:frechet}: a
afirma\c{c}\~ao ``$E_1 = \mathcal{O}(\alpha^{2})$'' \emph{\'e} a
verifica\c{c}\~ao emp\'irica de que o resto \'e $o(\norm{\delta m})$ e de que
o operador usado \'e de fato $F'$.
...
\begin{teoria}[Duas formula\c{c}\~oes equivalentes, com consequ\^encias
algor\'itmicas diferentes]
...
inc\'ognitas conjuntas, com $\mathcal{A}(m)u = f$ como restri\c{c}\~ao
relaxada:
\[
\min_{m,u}\ \tfrac12\norm{Ru-d_{\text{obs}}}^{2}
...
\'e propriedade da \emph{geometria de $J$} e n\~ao melhora com melhor
regulariza\c{c}\~ao.
...
\end{tabular}
\end{center}
Regularizar mais \emph{n\~ao} tira uma invers\~ao de um m\'inimo local: apenas
deixa o modelo errado mais suave.
\end{armadilha}
...
Por que essa distin\c{c}\~ao \'e exatamente o que o teste de Taylor mede?
...
linear em $\delta m$. O teste de Taylor verifica a taxa $\mathcal{O}(\alpha^2)$,
isto \'e, exatamente a uniformidade do resto. (2)
...
componentes independentes (simetria trigonal do tensor mais a existencia de
uma energia de deforma\c{c}\~ao). Impor simetrias reduz esse n\'umero:
...
tempo}. Escrever ``$+s$'' em \eqref{eq:ac2} \'e redefinir a fonte --- pr\'atica
...
$\mathcal{O}(8)$ com $G=5$, $C=0{,}5$: a curva espacial promete $0{,}07\%$ e o
esquema entrega $1{,}6\%$ --- o erro espacial sumiu e sobrou o temporal,
sozinho.

Verifica\c{c}\~ao independente no propagador do curso: velocidade de fase
relativa medida $1{,}0146$, contra $1{,}0160$ da rela\c{c}\~ao completa e
...
\textbf{A observa\c{c}\~ao de partida.} Uma onda plana harm\^onica
$e^{\mathrm{i}kx}$ \'e uma fun\c{c}\~ao \emph{inteira} de $x$: faz sentido
avali\'a-la em $x$ complexo. E se $x \to x + \mathrm{i}\eta$ com $\eta > 0$ e
$k > 0$,
\[
...
\]
e substitua $\partial_x \to s(x)^{-1}\partial_x$ na equa\c{c}\~ao. Onde
...
matched}. A reflex\~ao que se observa na pr\'atica vem inteiramente da
\textbf{discretiza\c{c}\~ao} de $\sigma$ --- por isso $\sigma$ \'e crescido
suavemente (tipicamente polinomial) em vez de ligado de uma vez.
\end{matematica}
...
\textbf{A CPML} (\emph{convolutional} PML) generaliza o estiramento para
\[
s(x) = \kappa(x) + \frac{\sigma(x)}{\alpha(x) + \mathrm{i}\omega},
\qquad \kappa \ge 1,\ \alpha > 0 .
\]
O $\kappa$ estica tamb\'em a parte \emph{real} da coordenada, o que encurta o
comprimento de onda aparente dentro da moldura e ataca diretamente o caso
rasante. O $\alpha$ --- o \emph{deslocamento de frequ\^encia} --- amortece as
componentes evanescentes e de baix\'issima frequ\^encia, que a PML cl\'assica
trata mal.
...
regra $z_{\max}\approx x_{\text{off}}^{\max}/3$ a $/2$, que n\~ao depende da
densidade.
\end{teoria}
...
A corre\c{c}\~ao \'e aproximada, e exata s\'o para meio homog\^eneo.
\end{atencao}
...
$\tau \approx 0{,}90\,T$. A regra de meio ciclo ($T/2$) \'e portanto
...
Seja $\tau^{*}$ o atraso que maximiza a correla\c{c}\~ao entre sint\'etico e
observado numa janela, e $J = \tfrac12\sum (\tau^{*})^{2}$. A fonte adjunta
(Luo \& Schuster; Tromp \emph{et al.}) \'e
\begin{equation}
f_{\text{adj}}^{\text{cc}} = \frac{\tau^{*}}{N}\,\partial_t d_{\text{cal}},
\qquad
N = \int d_{\text{cal}}\,\partial_{tt}d_{\text{cal}}\dd t .
\end{equation}
...
$\Phi(\tau) = \tau^{2}$ exatamente: perfeitamente convexa em deslocamento,
dentro da janela.
...
transformar por $d \mapsto e^{\beta d}$ ou $d\mapsto d^{2}$ (n\~ao lineares, e
a convexidade em deslocamento \emph{n\~ao} \'e preservada por uma
transforma\c{c}\~ao arbitr\'aria); ou aplicar OT no \emph{gr\'afico} do
...
Duas implementa\c{c}\~oes s\~ao poss\'iveis, e a escolha \'e puramente de
custo:

\textbf{(i) Integral de espalhamento.} Calcule e armazene
$G(\mathbf{x},\mathbf{x}_r)$ para \emph{cada} receptor --- $n_r$
fun\c{c}\~oes de Green --- e some. Custo: $n_r$ modelagens por tiro.
...
res\'iduos como fontes simult\^aneas. Custo: \textbf{1} modelagem por tiro.

A economia \'e $n_r$ --- t\^ipicamente $10^{2}$ a $10^{3}$. \textbf{O
...
$(\nabla^{2})^{*} = \nabla^{2}$ com contorno homog\^eneo. Um operador s\'o com
derivadas de ordem \emph{par} \'e auto-adjunto.
...
Escrevendo $G \sim A\,e^{\mathrm{i}\omega T}$, o produto dos dois conjugados
carrega a fase $-\omega[T_s(\mathbf{x}) + T_r(\mathbf{x})]$, e o res\'iduo
carrega $+\omega t$. A fase total de \eqref{eq:kernel} \'e portanto
$\omega\,[\,t - T_s(\mathbf{x}) - T_r(\mathbf{x})\,]$ --- e \'e a sua
...
\emph{banana-doughnut}. N\~ao \'e curiosidade: contradiz frontalmente a
...
Taylor & que $\mathbf{g}$ \'e a derivada de Fr\'echet do $J$ que o c\'odigo
  avalia & erro de escala comum a $J$ e $\mathbf{g}$ \\
...
\emph{um} par\^ametro de cada vez, e depois em combina\c{c}\~ao. Erros de regra
da cadeia s\'o aparecem no segundo caso.
\end{enumerate}
\end{pratica}
...
mesmo que aplicar o adjunto. (3) Fator $2$: ou a diferen\c{c}a centrada foi
implementada com $2\alpha$ onde devia ser $\alpha$ (ou vice-versa), ou o
funcional foi definido com $\tfrac12\norm{r}^{2}$ e o gradiente derivado de
$\norm{r}^{2}$.
\end{exercicio}
...
(diagonal) e \textbf{desfoca} a imagem (fora da diagonal) --- \'e por isso
...
O primeiro termo do lado direito de (4) gera $H_1$; o segundo gera $H_2$.
\textbf{Omitir o termo $-\delta m\,\partial_{tt}\lambda$ em (4) produz
exatamente $H_{\text{GN}}$} --- uma linha de c\'odigo separa os dois
m\'etodos, o que \'e conveniente e tamb\'em uma fonte de confus\~ao na
literatura.
...
itera\c{c}\~ao. O gradiente resultante \'e um \emph{estimador n\~ao viesado}
do gradiente completo: $\mathbb{E}[\mathbf{g}_{\text{sub}}] = \mathbf{g}$. A
...
\textbf{Codifica\c{c}\~ao de fontes (\emph{source encoding}).} Some todos os
...
\textbf{O pre\c{c}o, em ambos:} os termos cruzados n\~ao s\~ao zero em cada
realiza\c{c}\~ao --- s\~ao ru\'ido de \emph{crosstalk} que decai como
$1/\sqrt{n_{\text{real}}}$. E a codifica\c{c}\~ao exige que todos os tiros
...
\item A PSF num ponto profundo sai alongada na vertical. Que propriedade da
...
custa apenas uma substitui\c{c}\~ao --- $\mathcal{O}(N)$. \textbf{Com muitos
tiros, o custo por tiro tende a zero.} Essa \'e a vantagem estrutural do
...
existentes s\~ao 2D, de baixa dimens\~ao (modelo reparametrizado com poucas
dezenas a poucos milhares de par\^ametros) ou com substitutos
(\emph{surrogates}). Trate afirma\c{c}\~oes de ``UQ completa'' em 3D com
...
$\chi(t)$ se anula para $t<0$, ent\~ao
$\chi(\omega) = \int_0^{\infty}\chi(t)e^{\mathrm{i}\omega t}\dd t$ converge e
\'e \textbf{anal\'itica no semiplano superior} $\operatorname{Im}\omega > 0$ ---
porque ali $e^{\mathrm{i}\omega t}$ decai para $t>0$. \'E a causalidade, e
nada mais, que produz a analiticidade.

\textbf{Kramers--Kronig.} Aplicando a f\'ormula integral de Cauchy a uma
fun\c{c}\~ao anal\'itica no semiplano superior, as partes real e imagin\'aria
...
Efeito sobre uma Ricker de 20\,Hz ap\'os 1500\,m, medido no curso:
...
diagn\'osticos (§11.5) mas n\~ao existe crit\'erio autom\'atico que decida se
...
\item A PSF \'e a \'unica medida local de resolu\c{c}\~ao que n\~ao exige
conhecer a verdade --- e portanto a \'unica que serve em dado real.
...
$-6$\,dB em $1{,}63f_0$; $-20$\,dB em $2{,}21f_0$ (ra\'izes de
