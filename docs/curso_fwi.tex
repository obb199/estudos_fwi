...
III & 7--10  & O problema inverso: função objetivo, estado adjunto, verificação \\
IV  & 11--13 & Otimização, FWI completa, regularização \\
V   & 14     & Onde a física adotada deixa de valer \\
\bottomrule
\end{tabular}
\end{center}
...
NumPy, com cerca de 200 linhas. Não usa GPU, não é rápido, e é inteiramente
transparente: cada termo está visível. É com ele que derivamos e
...
Capítulo 13 --- o texto discute o resultado obtido, não o esperado.
\end{atencao}
...
diferença de fase maior que meio período. É o Capítulo 8.
...
pela abertura da aquisição (regra prática: metade a um terço do maior
\emph{offset});
...
\item $\sigma_{xx} = \sigma_{zz} = \lambda\,(\partial_x v_x + \partial_z v_z)$:
as tensões normais ficam \emph{iguais}. O tensor vira isotrópico;
...
trivial, e o gradiente adjunto sai limpo (Capítulo 9). A conversão para
...
O atraso $t_0 = 1/f_0$ torna a wavelet praticamente causal.
...
\begin{equation}
\lambda_{\min} = \frac{c_{\min}}{f_{\max}}, \qquad
\text{resolução} \approx \frac{\lambda_{\min}}{2}.
\end{equation}
...
\caption{Esquerda: curva de dispersão numérica por ordem espacial. Direita:
traço a 2{,}4\,km da fonte em meio homogêneo, com $G=9{,}5$ e $G=2{,}4$.
A cauda no traço vermelho não existe na física.}
\end{figure}
...
3. dt     = 0.9 * C_lim * dh / c_max       # margem de 10%
...
Cond. absorvente & equação unidirecional    & baixo & boa a 90$^\circ$ \\
...
A comparação é eloquente: a CPML com 10 pontos já supera o Cerjan com 45.
...
auto-adjunto. Isso preserva a validade do teste de gradiente do Capítulo 10
...
que cruzam a direta a partir de um \emph{offset} crítico. Dominam o
...
\textbf{3.} \texttt{inpa['acq\_type']} tem de ser 1 (superfície) ou 2
(\emph{crosswell}). Qualquer outro valor --- 0, por exemplo --- \emph{não}
levanta erro: o código mapeia os receptores errado e devolve um sismograma
plausível e errado. Veja a Seção \ref{sec:validacao}.
\end{armadilha}
...
\midrule
0 & \texttt{taux}, \texttt{tauz} (média) & \textbf{pressão}: $-(\sigma_{xx}+\sigma_{zz})/2$ \\
...
A causa era \texttt{acq\_type = 0}. O PyFWI aceita esse valor sem reclamar,
mas só trata 1 e 2.

Duas lições, e a segunda vale mais:
\begin{enumerate}
\item Em aquisição de superfície, \texttt{acq\_type = 1}. Sempre.
...
\texttt{sdo} & não (2) & ordem espacial \emph{real}: 4 ou 8 \\
\texttt{npml} & não (0) & espessura da CPML em pontos \\
\texttt{pmlR} & com \texttt{npml} & coef.\ de reflexão teórico (ex.\ 1e-5) \\
\texttt{pml\_dir} & com \texttt{npml} & 0 = só $x$, 1 = só $z$, 2 = ambas \\
\texttt{acq\_type} & não (1) & 1 = superfície, 2 = \emph{crosswell} \\
...
\begin{enumerate}[leftmargin=*]
\item \textbf{O sinal.} A atualização é $\mathbf{m} \leftarrow \mathbf{m} -
\alpha\,\mathbf{g}$. Onde $g$ é negativo, a velocidade vai \emph{aumentar}.
Confira sempre o sinal contra uma perturbação conhecida: é o teste de sanidade
mais barato.
...
\item \textbf{Os arcos (``banana'').} Um tiro produz uma faixa larga ligando
fonte e receptor, não um ponto. É a zona de Fresnel: a onda é sensível a um
\emph{volume}. Resolução finita é consequência direta disso.
...
A Hessiana de Gauss--Newton tem, na diagonal, exatamente a \textbf{energia do
campo} em cada ponto --- que é a iluminação. Por isso dividir o gradiente pela
energia do campo direto \emph{não} é um truque cosmético: é uma aproximação
diagonal legítima de $H^{-1}$. É o que o PyFWI chama de
...
\addcontentsline{toc}{part}{Parte IV --- Otimização e FWI completa}
...
consome tempo de máquina --- a vantagem diminui, porque as iterações do
l-BFGS gastam mais avaliações na busca linear.
...
A vantagem da multiescala cresce exatamente quando o modelo inicial piora ---
e é por isso que o exercício de degradar o inicial de propósito é o que
realmente mede o valor da estratégia. Um experimento que só mostra ganho
quando o adversário já estava bem não demonstra grande coisa.
\end{atencao}
...
degradar o inicial de propósito. Quatro modelos iniciais, mesmo fluxo, 12
iterações no total em ambos os casos:
...
\begin{teoria}[Três leituras, e a segunda é a mais importante]
...
Regularização \textbf{não} melhora o ajuste dos dados. Ela sempre \emph{piora}
$J_{dados}$, por construção. O que ela faz é escolher, entre os muitos modelos
...
\midrule
$\infty$ & $3{,}2\times10^{-5}$ & 1{,}0000 & 20{,}0 \\
...
\item ASNAASHARI, A.; BROSSIER, R.; GARAMBOIS, S.; THORE, P.; VIRIEUX, J.
...
for full-waveform inversion and reservoir monitoring.} SoftwareX, 2023.
...
ZHOU, W. \emph{An Introduction to Full Waveform Inversion.} Society of
Exploration Geophysicists, 2017.
\end{enumerate}
