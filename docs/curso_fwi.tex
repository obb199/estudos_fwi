...
NumPy, com 476 linhas --- metade delas comentário. Não usa GPU, não é rápido, e
...
O repositório traz \arq{testes/verificar\_fisica.py}, com 29 verificações que
...
regularização, e o modelo constant-$Q$.

Elas rodam em poucos minutos:
...
\begin{atencao}[Uma precisão sobre a curva de dispersão]
...
erro próprio, e o detalhe interessante é que ele tem sinal \textbf{oposto}: o
\emph{leap-frog} adianta a fase, o estêncil espacial atrasa, e os dois se
cancelam parcialmente.

Medido para O(4) com $G=6$: a curva semi-discreta dá $0{,}9939$, enquanto o
esquema completo dá $0{,}9980$ ($C=0{,}3$) ou $1{,}0055$ ($C=0{,}5$). O erro
real é \emph{menor} que o previsto pela curva espacial sozinha --- ela é uma
estimativa conservadora, que é exatamente o que se quer para dimensionar malha.
\end{atencao}
...
semi-discreto). Direita: traço a 2{,}4\,km da fonte em meio homogêneo, com
...
\emph{pseudo-Hessiano} de Shin \emph{et al.} (2001), obtido desprezando as
contribuições cruzadas entre pontos do modelo.

É uma aproximação, não uma identidade --- a diagonal exata de $J'^{T}J'$
envolve também a função de Green até os receptores. Mas é boa o bastante, e
